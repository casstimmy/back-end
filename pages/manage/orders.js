...
const handleSendEmail = async () => {
  const email = selectedOrder?.shippingDetails?.email || selectedOrder?.customer?.email;
  if (!email) {
    console.error("No email address found for this order");
    return;
  }
  setSendingEmail(true);

  const customerData = {
    name: selectedOrder.shippingDetails?.name || selectedOrder.customer?.name || "Customer",
    orderId: selectedOrder._id,
    status: selectedOrder.status,
    total: selectedOrder.total ?? 0, // Pass number, not formatted string
            ...
            <div className="bg-white rounded-lg p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto relative shadow-lg">
              ...
              <div>
                ...
                <p className="text-sm mt-2">
                  ...
                  {selectedOrder.shippingDetails?.email}<br />
