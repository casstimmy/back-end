...
export default function PromotionManagement() {
  ...
  const [customPromoType, setCustomPromoType] = useState("");
  ...
  const handleUpdateClick = async (_id) => {
    try {
      ...
      const updateData = {
        ...editablePromotion,
        _id,
        promoStart: editablePromotion.promoStart ? new Date(editablePromotion.promoStart).toISOString() : null,
        promoEnd: editablePromotion.promoEnd ? new Date(editablePromotion.promoEnd).toISOString() : null,
      };

      console.log("Updating promotion with data:", updateData);

      await axios.put("/api/products", updateData);
  ...
  const handleSavePromotion = async () => {
    ...
    const finalPromoType = customPromoType.trim() || promoType;
    try {
      await axios.put("/api/products", {
        ...
        promoStart: promoStart ? new Date(promoStart).toISOString() : null,
        promoEnd: promoEnd ? new Date(promoEnd).toISOString() : null,
      });
      fetchPromotions();
      closeModal();
    } catch (err) {
        ...
        <table className="min-w-full border border-amber-100 shadow-xl rounded-sm overflow-hidden">
          <thead className="bg-gradient-to-r from-amber-500 to-amber-600 text-white">
            <tr>
              ...
              <th className="p-4">Promo Type</th>
                ...
                <tr key={promo._id}>
                  ...
                  <td className="p-4">
                    {editIndex === index ? (
                      <div className="flex flex-col gap-2">
                        <select
                          value={promotionTypeOptions.includes(editablePromotion.promoType) ? editablePromotion.promoType : "custom"}
                          onChange={(e) => {
                            if (e.target.value === "custom") {
                              setEditablePromotion((prev) => ({ ...prev, promoType: prev.promoType }));
                            } else {
                              setEditablePromotion((prev) => ({ ...prev, promoType: e.target.value }));
                            }
                          }}
                        ...
                        {(promotionTypeOptions.includes(editablePromotion.promoType) === false && editablePromotion.promoType) && (
                          <input
                            value={editablePromotion.promoType}
                            onChange={(e) => setEditablePromotion((prev) => ({ ...prev, promoType: e.target.value }))}
                            type="text"
                            placeholder="Enter custom type"
                            className="border p-2 rounded-md w-40 text-sm"
                          />
                        )}
                      </div>
                    ) : (
                  ...
                  <td className="p-4">
                    {editIndex === index ? (
                      <input
                        name="promoStart"
                        value={editablePromotion.promoStart ? editablePromotion.promoStart.split('T')[0] : ""}
                        onChange={handleChange}
                        type="date"
                        className="w-32 border p-2 rounded-md"
                      />
                  ...
                  <td className="p-4">
                    {editIndex === index ? (
                      <input
                        name="promoEnd"
                        value={editablePromotion.promoEnd ? editablePromotion.promoEnd.split('T')[0] : ""}
                        onChange={handleChange}
                        type="date"
                        className="w-32 border p-2 rounded-md"
                      />
                    ) : promo.promoEnd ? (
                      new Date(promo.promoEnd).toLocaleDateString()
                    ) : (
                      "--"
                    )}
                  </td>
...
{isModalOpen && (
  <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4">
    <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md sm:max-w-lg max-h-[80vh] overflow-y-auto">
      ...
      <div className="mb-4">
        ...
        {promoType === "custom" && (
          <input
            ...
            placeholder="Enter custom promotion type"
            value={customPromoType}
            onChange={(e) => setCustomPromoType(e.target.value)}
          />
        )}
      </div>
      ...
      {/* Promo Start Date */}
      <div className="mb-4">
        <label className="block mb-2 font-medium">Promotion Start Date</label>
        <input
          type="date"
          className="border w-full p-2 rounded-md"
          value={promoStart}
      ...
      {/* Promo End Date */}
      <div className="mb-4">
        <label className="block mb-2 font-medium">Promotion End Date</label>
        <input
          type="date"
          className="border w-full p-2 rounded-md"
          value={promoEnd}
