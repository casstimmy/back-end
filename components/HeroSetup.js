...
export default function HeroSetup() {
  ...
  const addOrUpdateHeroPage = async () => {
    ...
    // Determine final promotion: use custom if filled, otherwise use selected
    let finalPromotion = "";
    if (customPromotion.trim()) {
      finalPromotion = customPromotion.trim();
    } else if (promotion && promotion !== "custom") {
      finalPromotion = promotion;
    }

    // Calculate final CTA link: if promotion is set, generate dynamic link, otherwise use manual link
    let finalCtaLink = ctaLink;
    if (finalPromotion) {
      finalCtaLink = `/shop/shop?promo=yes&promoType=${encodeURIComponent(finalPromotion)}`;
    }
  ...
  const editHeroPage = (hero) => {
    ...
    // Check if promotion is in predefined list or custom
    const heroPromotion = hero.promotion || "";
    
    if (heroPromotion && promotionOptions.includes(heroPromotion)) {
      setPromotion(heroPromotion);
      setCustomPromotion("");
    } else if (heroPromotion) {
      setPromotion("custom");
      setCustomPromotion(heroPromotion);
    } else {
      setPromotion("");
      setCustomPromotion("");
    }
  ...
  // Generate dynamic CTA link based on promotion type
  const getCtaLink = (hero) => {
    if (hero.promotion) {
      return `/shop/shop?promo=yes&promoType=${encodeURIComponent(hero.promotion)}`;
    }
    return hero.ctaLink || "/shop/shop";
  };
                  ...
                  >
                    ...
                    <div className="relative flex flex-col md:flex-row items-center justify-between max-w-3xl mx-auto px-6 w-full">
                      ...
                      >
                        ...
                        {currentHero.promotion && (
                          <p className="text-sm md:text-base font-semibold mb-2 text-yellow-300 uppercase tracking-wider">
                            {currentHero.promotion}
                          </p>
                        )}
          ...
          <div className="space-y-5 w-full max-w-4xl bg-white shadow-2xl rounded-xl py-10 p-6">
            ...
            <div className="flex flex-col space-y-2">
              <label className="text-sm font-semibold text-gray-700">
                CTA Link {promotion && "(Auto-generated from Promotion)"}
              </label>
              <input
                type="text"
                value={(() => {
                  const finalPromo = customPromotion.trim() || (promotion && promotion !== "custom" ? promotion : "");
                  if (finalPromo) {
                    return `/shop/shop?promo=yes&promoType=${encodeURIComponent(finalPromo)}`;
                  }
                  return ctaLink;
                })()}
                onChange={(e) => {
                  // Only allow manual edit if no promotion is set
                  if (!promotion) {
                    setCtaLink(e.target.value);
                  }
                }}
                placeholder="/shop/shop"
                disabled={!!(promotion || customPromotion)}
              ...
              {promotion && (
                <p className="text-xs text-green-600 italic">
                  ✓ Link auto-generated from promotion type
                </p>
              )}
            </div>
